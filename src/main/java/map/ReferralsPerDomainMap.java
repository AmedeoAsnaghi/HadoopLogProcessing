...
public class ReferralsPerDomainMap extends MapReduceBase implements Mapper<LongWritable, Text, Text, IntWritable> {
    ...
    @Override
    public void map(LongWritable key, Text text, OutputCollector<Text, IntWritable> outputCollector, Reporter reporter) throws IOException {
                ...
                for(int j=0; littleTokenizer.hasMoreTokens(); j++){
                    ...
                    if(j==0){
                        if(domain.equals("\"http:") || domain.equals("\"https:")){
                            ...
                            if(domainOk){
                                outputCollector.collect(new Text(finalDomain), new IntWritable(1));
                                return;
                            }
                        }
                    }
